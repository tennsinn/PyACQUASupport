from ACQUAlyzer import Variables, Smd, SubProject
...
pkg_na_key = {'3GPP':'3GPP', 'BigRules':'BigRules', 'ATT':'ATT', 'AT&T':'ATT', 'TMO':'TMO', 'T-Mobile':'TMO'}
pkg_uc_key = {'Handset':'HA', 'HA':'HA', 'Headset':'HE', 'HE':'HE', 'Handsfree':'HH', 'Hands-free':'HH', 'HH':'HH', 'Bluetooth':'BT', 'BT':'BT'}
pkg_bw_key = {'Narrowband':'NB', 'NB':'NB', 'Wideband':'WB', 'WB':'WB', 'Superwideband':'SWB', 'Super-Wideband':'SWB', 'SWB':'SWB'}

def set_seq_config():
    if not Variables.Exists(var_sys_labname):
        set_const_var()
    if not Variables.Exists(var_seq_name):
        set_seq_tag_by_title()
...
def set_seq_tag(seq_name, seq_uc, seq_bw):
    save_var(var_seq_name, seq_name, const.evsUserDefined, '', '', Smd.Title, False)
    save_var(var_dut_uc, seq_uc, const.evsUserDefined, '', '', Smd.Title, False)
    save_var(var_call_bw, seq_bw, const.evsUserDefined, '', '', Smd.Title, False)

def set_seq_tag_by_title():
    title = SubProject.Title
    seq_name = 'Default'
    seq_uc = 'All'
    seq_bw = 'All'
    for key in pkg_na_key.keys():
        if key in title:
            seq_name = pkg_na_key[key]
    for key in pkg_uc_key.keys():
        if key in title:
            seq_uc = pkg_uc_key[key]
    for key in pkg_bw_key.keys():
        if key in title:
            seq_bw = pkg_bw_key[key]
    set_seq_tag(seq_name, seq_uc, seq_bw)

def set_test_config():
    seq_name = get_var_value(var_seq_name)
    seq_uc = get_var_value(var_dut_uc)
    seq_bw = get_var_value(var_call_bw)
    usecase_set = usecase.copy()
    if 'All' != seq_uc and seq_uc in ['HA', 'HE', 'HH', 'BT', 'HI']:
        usecase_set.update({'GrayOut': {seq_test_mode: ['single','loop']}})
    if 'All' != seq_bw and seq_bw in ['NB+WB', 'NB', 'WB', 'SWB', 'FB', 'Wechat', 'Teams']:
        usecase_set.update({'GrayOut': {seq_test_mode: ['single','loop']}})
    if 'TMO' == seq_name:
        ...
        save_var(var_test_net_flow, 'LTE_AMR_NB+WB,LTE_EVS_NB+WB,WCDMA_AMR_NB+WB', const.evsUserDefined, '', '', Smd.Title, True)
    if 'ATT' == seq_name:
        network_att = network.copy()
        network_att.update({'Options': ['WCDMA', 'LTE'], 'Values': ['WCDMA', 'LTE']})
        bandwidth_att = bandwidth.copy()
        bandwidth_att.update({'Options': ['NB+WB', 'NB', 'WB'], 'Values': ['NB+WB', 'NB', 'WB']})
        usecase_set.update({'Options': ['All Usecase', 'Handset', 'Headset', 'Handsfree'], 'Values': ['All', 'HA', 'HE', 'HH'],})
        run_universal_questionnaire_gui(seq_test_mode, network_att, vocoder, bandwidth_att, usecase_set, hs_type)
        save_var(var_test_net_flow, 'LTE_AMR_NB+WB,LTE_EVS_NB+WB,WCDMA_AMR_NB+WB', const.evsUserDefined, '', '', Smd.Title, True)
